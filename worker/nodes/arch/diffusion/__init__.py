"""Architecture dispatch registry for the AnvilML worker diffusion backends.

This package provides dynamic diffusion architecture module loading and model-architecture
dispatch. On first import, ``_ensure_imported()`` scans the ``worker/nodes/arch/diffusion``
...
def _ensure_imported() -> None:
    """Import all sibling diffusion architecture modules exactly once.
    ...
    for mod_info in pkgutil.iter_modules(__path__):
        ...
        # The "worker.nodes.arch.diffusion." prefix is required — bare
        # module names like "zit" would cause ModuleNotFoundError.
        full_name = f"worker.nodes.arch.diffusion.{mod_info.name}"
