"""Flux 2 VAE architecture module — shape inference from safetensors header.

This module implements the full four-step loading contract
(`ANVILML_DESIGN.md §11.3`) for the Flux 2 VAE architecture family:
    ...
    2. can_handle(key) — implemented; returns True for "flux2".
    ...
    4. decode(vae_module, latent) — implemented in this task (P25-E1).

The module is independently implemented from ``zit_vae.py`` — each arch family
has its own formula and key patterns. This is the second VAE module in the
dispatcher, registered alongside ``zit_vae``.

This file corresponds to Phase 25 task P25-E1.
"""
...
# Flux2VaeModel falls back to plain `object` as its base in that case — the
...
# Architecture identifier — used by can_handle() and by dispatch.
ARCH: str = "flux2"
...
def _infer_hyperparams_inner(f: Any, path: str) -> dict[str, Any]:
    """Infer hyperparameters from the safetensors header of a Flux 2 VAE checkpoint.
    ...
    2. **Encoder channel count** — finds the first key matching the pattern
       ``encoder.blocks.N.conv.weight`` and extracts ``shape[1]`` (in_channels).
       If no such key exists, falls back to the ``xyz_encoder_block*conv``
       pattern and extracts ``shape[1]``.

    3. **Decoder channel count** — same logic as encoder but with
       ``decoder.blocks.N.conv.weight`` / ``xyz_decoder_block*conv``.
       For decoder, the output channel (shape[0]) is the true decoder_channels
       because the first block's output determines the decoder's output space.
    ...
    # Fallback: xyz_encoder_block*conv pattern (no-meta fixture).
    # The no-metadata fixture may or may not use a .weight suffix, so the
    # pattern matches both `xyz_encoder_block0_conv` and
    # `xyz_encoder_block0_conv.weight`.
    ...
    # For decoder, shape[0] is the output channel count. The first block's
    # output determines the decoder's output space.
    ...
    # Fallback: xyz_decoder_block*conv pattern (no-meta fixture).
    # Matches both with and without .weight suffix.
...
def _infer_hyperparams(path: str) -> dict[str, Any]:
    """Infer hyperparameters from a Flux 2 VAE checkpoint's safetensors header.
    ...
    Args:
        path: File system path to the Flux 2 VAE safetensors checkpoint.
...
class Flux2VaeModel(_ModuleBase):
    """Flux 2 VAE model constructed from convolutional layer-level building blocks.
    ...
    The ``.arch`` attribute is set to ``"flux2"`` after construction so that
    ...
    def __init__(self, hyperparams: dict[str, Any]) -> None:
        ...
        # interpolated channel counts produce non-standard values.
...
def _build_key_remapping(
    ...
    function handles two cases for the Flux 2 VAE key namespace:
    ...
    This function is built independently from ``zit_vae.py``'s diffusion key
    ...
    # Pattern-based remapping rules for VAE key naming conventions.
...
# ---------------------------------------------------------------------------
# Partial load — meta construction + dtype application (P25-E1)
# ---------------------------------------------------------------------------
...
def load(path: str, caps: dict, device: str = "cpu") -> Flux2VaeModel:
    """Construct the Flux 2 VAE model on meta-device, materialize, and load weights.
    ...
    3. Construct ``Flux2VaeModel`` on ``torch.device("meta")``, apply dtype,
    ...
    4. Set the ``.arch`` attribute to ``"flux2"``.

    Args:
        path: Filesystem path to a Flux 2 VAE-format safetensors checkpoint file.
        caps: Worker capability dict from ``probe_capabilities()`` with keys
            ``fp32``, ``fp16``, ``bf16``, ``fp8``, ``fp4``, ``flash_attention``
    ...
    Returns:
        A ``Flux2VaeModel`` instance with parameters materialized on *device*,
        carrying the selected dtype, and ``.arch == "flux2"``.
    ...
    if torch is None:
        raise RuntimeError(
            "flux2_vae.py: torch is not installed - load() is a real-mode-only "
    ...
    with torch.device("meta"):
        model = Flux2VaeModel(hyperparams)
    ...
    # This is a defensive measure: every parameter and buffer is zeroed
    # so that any checkpoint keys not covered by the remapping don't
    # contain garbage values when loaded. Loaded values are unaffected:
    # every matched key is overwritten by load_state_dict() below.
...
def decode(
    ...
    Args:
        vae_module: A loaded ``Flux2VaeModel`` instance with parameters
    ...
    if torch is None:
        raise RuntimeError(
            "flux2_vae.py: torch is not installed - decode() is a real-mode-only "
    ...
    # RGB mode takes the first 3 channels from the output.
...
def can_handle(key: str) -> bool:
    ...
    Args:
        key: Architecture string to check, e.g. ``"flux2"`` or
            ``"zit_vae"``.

    Returns:
        ``True`` if *key* equals ``"flux2"``, ``False`` otherwise.
    """
    return key == ARCH
