...
def test_load_mock_no_torch_import() -> None:
    ...
    Tests:
        ...
        module from cache, and re-import the ``clip_l`` module. Assert
    ...
    sys.modules.pop("worker.nodes.arch.clip.clip_l", None)
    ...
    try:
        ...
        import worker.nodes.arch.clip.clip_l as clip_l_mod

        importlib.reload(clip_l_mod)
        ...
        assert callable(clip_l_mod.can_handle)
        assert callable(clip_l_mod.load)
    finally:
