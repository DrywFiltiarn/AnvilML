...
def test_load_mock_no_torch_import() -> None:
    ...
    Tests:
        ...
        been loaded at all) and import the ``t5`` module in it. Assert
        the child exits 0 and reports that ``torch`` is still absent
        from its own ``sys.modules`` after the import — proving no
    ...
    """
    script = (
        "import worker.nodes.arch.clip.t5; "
        "import sys; "
