...
**Pre-existing in the starting repository (NOT created by these tasks):** `README.md`, `LICENSE`, `CODE_OF_CONDUCT.md` and any similar front-matter files; the `docs/` directory (`ARCHITECTURE.md`, `ENVIRONMENT.md`, `PHASES.md`, and the `TASKS_PHASE*.md` set); the `docs/FORGE_AGENT_RULES.md` file; and the `.forge/` reports/state structure. The `docs/`, `docs/FORGE_AGENT_RULES.md`, and `.forge/` contents are mandatory for `forge.py` to start at all, so they are guaranteed present before the first session runs. Cline must not attempt to create or overwrite any of these.
