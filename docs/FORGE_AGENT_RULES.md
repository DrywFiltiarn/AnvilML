...
**Single write rule:** Write the plan or implementation report file EXACTLY ONCE, in a single
write operation, containing the complete finished document. Do not write interim notes,
progress updates, or partial drafts to the report file. The report must not exist until it is
complete and ready. A report that does not begin with `# Plan Report: <TASK_ID>` or
