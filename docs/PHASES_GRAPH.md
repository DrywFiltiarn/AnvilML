...
**Depends on:** Phase 1, 2, 3 (transitively), Phase 5 (`detect_all_devices()`), Phase 6 (`ModelStore`, `ModelScanner`), Phase 8 (`WorkerPool`; `P18-D2` extracts from its `spawn_all()`, `P18-D3` composes `request_shutdown()` with that extraction), Phase 14 (`AppState`'s existing fields), Phase 16 (`AppState.broadcaster`), Phase 17 (cancellation, which `DELETE /v1/jobs` composes alongside).

**Implements:** `AppState` gains its **final** two `§13.2` fields — `hardware`, `env_report` (`P18-A1`); `GET /v1/system`/`GET /v1/system/env` (`P18-B1`), then `GET /v1/system/versions` + `ComponentVersions` type (`P18-B2`); `AppState` gains `model_store`; `GET /v1/models`/`GET /v1/models/:id` (`P18-C1`); `POST /v1/models/rescan` (`P18-C2`); **`P18-C3` — startup model scan, added by retrofit** (see below); `GET /v1/workers` (`P18-D1`); `WorkerPool::spawn_worker()`, a reusable single-worker spawn extracted from `spawn_all()`'s one-shot bulk loop (`P18-D2`, retrofit-repurposed — see below); `POST /v1/workers/:id/restart`, explicitly composing `request_shutdown()` with `spawn_worker()` rather than assuming a side-effect respawn (`P18-D3`, retrofit-added — see below); `DELETE /v1/jobs/:id` (`P18-E1`) then `DELETE /v1/jobs` bulk clear (`P18-E2`); real OpenAPI generation from `utoipa` annotations, replacing Phase 1's stub (`P18-F1`); the `openapi-drift` CI gate wired to real generation + diff check, replacing Phase 1's placeholder (`P18-F2`); this phase's Runnable Proof (`P18-G1`).
...
- `P18-D2`'s original text (the restart handler) assumed `request_shutdown()` plus Phase 8's respawn-on-crash machinery composed into a restart, and further assumed that composition would simply become true once `P8-E4`/`P8-E6` wired `RespawnPolicy` into a real respawn loop. **Both assumptions were wrong, confirmed against the live repo during P18 planning, independent of whether `P8-E4`/`P8-E6` had landed:** `request_shutdown()` drives `ManagedWorker::run()`'s `RunOutcome::ShutdownRequested` arm, which calls `graceful_shutdown_child()` and then `break`s the supervising task permanently — a structurally different arm from `RunOutcome::Crashed{should_respawn:true}`, the only arm any Phase 8 respawn fix ever wired. `request_shutdown()` alone was never going to restart a worker, regardless of Phase 8's crash-respawn state. **Fixed by repurposing `P18-D2`** into a pure extraction (`WorkerPool::spawn_worker()`, pulled out of `spawn_all()`'s one-shot bulk loop so a single replacement worker can be spawned into an existing slot) **and adding `P18-D3`** as the actual restart handler: shut the old worker down, await it, then explicitly call `spawn_worker()` — composition of two real steps, not an assumed side effect of one. `P18-D1`'s `defers_to` was updated from `P18-D2` to `P18-D3` accordingly.
...
| 26 | `POST /v1/workers/:id/restart`'s premised respawn | `P18-D2` (original text) | The task text claimed `request_shutdown()` plus Phase 8's existing respawn-on-exit path composed into a restart with no new logic needed. `ManagedWorker::run()`'s `match` on `RunOutcome` sends `ShutdownRequested` down a `graceful_shutdown_child()`-then-`break` arm, structurally separate from the `Crashed{should_respawn:true}` arm that alone loops back to respawn — `request_shutdown()` was never going to trigger a restart, independent of Phase 8's crash-respawn wiring state (items 14/15 above). Confirmed against live `main` HEAD (post-`P17` verification) during P18 planning, before any P18-D-group task had executed. | In-phase correction (Phase 18 had not executed). Original `P18-D2` repurposed into a pure extraction, `WorkerPool::spawn_worker()`, pulled out of `spawn_all()`'s bulk-construction loop; new task `P18-D3` added as the actual restart handler, explicitly composing `request_shutdown()` (awaited) with `spawn_worker()`. `P18-D1`'s `defers_to` updated from `P18-D2` to `P18-D3`. |
...
| 27 | `backend`'s `mock-hardware` feature reaching `anvilml-worker`'s own `mock-hardware` flag | `backend/Cargo.toml` (initial feature declaration, pre-Phase-14) | `backend`'s `mock-hardware` feature forwarded only to `anvilml-scheduler/mock-hardware` (→ `anvilml-hardware/mock-hardware`). `anvilml-worker` declares its own separate `mock-hardware` Cargo feature — a distinct per-package flag despite the shared name — and `pool.rs` gates its real-vs-mock worker spawn decision on that flag directly. `backend`'s feature never listed `anvilml-worker/mock-hardware`, so `cargo build --features mock-hardware` at the binary level left workers attempting real-hardware startup regardless. Masked in practice by the pre-existing, legitimate `ANVILML_FORCE_WORKER_MOCK` runtime escape hatch (`pool.rs`), which `P14-E1` used to route around it without diagnosing or fixing the root Cargo.toml gap — found during a `P14-E1` implement-report review, ahead of `P16-E1`/`P17-D1`, both of which also run the built binary "under mock-hardware" with no mention of the env var. | Manual retrofit patch (not a numbered task) — `backend/Cargo.toml`'s `mock-hardware` feature now also forwards to `anvilml-worker/mock-hardware`; `ANVILML_FORCE_WORKER_MOCK` remains available as an independent runtime override, no longer load-bearing for the standard `--features mock-hardware` build path. |
| 28 | `pool_tests.rs`'s `handles()`-return-type dependency | `P8-G1` (`pool_tests.rs`, Phase 8) | `P18-D2`'s own acceptance text assumed `spawn_worker()` would be a "pure extraction — no behavior change… existing `pool_tests.rs` must pass unmodified." That assumption held for the extraction itself, but not for a second requirement discovered while planning `P18-D3`: `AppState.workers` is a bare `Arc<WorkerPool>` (no lock), so `P18-D3`'s restart handler needs `spawn_worker()`/a handle-replace path callable through a shared `&self`, not the `&mut self` `P18-D2`'s text specified. Giving `WorkerPool.handles` interior mutability (`std::sync::RwLock`) to enable that forced `handles()`'s return type to change from `&[WorkerHandle]` to an owned `Vec<WorkerHandle>` snapshot (a lock guard can't outlive the `&self` borrow the old signature promised, and returning one would let `anvilml-scheduler`'s hot-path `dispatch_one()` hold a blocking guard across `.await`). Every call site across the workspace was source-compatible with that change except one: `pool_tests.rs`'s `test_spawn_all_creates_one_handle_per_device` collected `&str` (borrowed from `handles()`'s return value) into a `let`-bound `Vec<&str>` used in a later statement — a pattern that requires the borrow's source to outlive that statement, which an owned temporary cannot. Surfaced as a real `cargo test` compile failure (`E0716`) when `P18-D2`/`P18-D3`'s combined patch was applied and built locally, not caught during planning (no Rust toolchain available in the planning/authoring session). | In-phase correction, applied as a one-line follow-up patch once the compile failure was reported. `pool_tests.rs`'s one affected line now collects owned `String`s (`h.worker_id.clone()`) instead of borrowed `&str` (`h.worker_id.as_str()`) — same assertion, same coverage, no logic change; `assert_eq!` against `vec!["0","1","2"]` still holds via `String: PartialEq<&str>`. No other call site in the workspace needed changes. |
| 29 | `anvilml.toml`'s `[[model_dirs]]` | `P900-A6`-era `anvilml.toml` (Phase 900) | Shipped with `path = "./models"` commented out entirely — no model directory was ever configured, so the scanner (`P6-A4`) found nothing to register on startup, and `infer_kind()`'s exact-leaf-name matching (`diffusion`/`text_encoders`/`vae`) wasn't documented anywhere near the config. Never caught because no phase before `P24-F1` submitted a real job requiring the registry to actually resolve a `model_id`. | Manual retrofit patch (not a numbered task), found and fixed while preparing to run `P24-F1`'s Runnable Proof — three explicit `[[model_dirs]]` entries added for `./models/diffusion`, `./models/text_encoders`, `./models/vae`. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 30 | `worker/executor.py`'s node-output reference resolution | `P17-B2` (`execute_graph()`) | `execute_graph()` passed each node's raw `inputs` dict straight to `execute()`, including `{"node_id", "output_slot"}` reference dicts — the format every non-trivial edge in `ANVILML_DESIGN.md` Appendix B.2's example graph uses. Every downstream node beyond the first would have received a literal reference dict instead of the referenced node's actual output value. No task before `P24-F1` submitted a graph with more than one real inter-node dependency through this exact path. | Manual retrofit patch (not a numbered task) — `execute_graph()` now resolves each `{"node_id", "output_slot"}`-shaped input against the accumulated `results` dict before calling `execute()`; scalars pass through unchanged. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 31 | `ctx.pipeline_cache` process-vs-job scope | `worker_main.py`'s `_dispatch_loop()` (Phase 17/19-era) | Originally `None` (unconditional `AttributeError` on the first loader node of any real job); an intermediate fix constructed a fresh `PipelineCache()` inside `run_execute()`'s `ctx_factory` closure — once per `Execute` message, not once per worker process, contradicting `pipeline_cache.py`'s own module docstring ("a single worker process lifetime") and silently defeating `LoadModel`/`LoadVae`/`LoadClip`'s entire purpose in calling `ctx.pipeline_cache.get_or_load()`. | Manual retrofit patch (not a numbered task) — `PipelineCache()` now constructed once in `_dispatch_loop()` before its `while True:` message loop; every job's `ctx_factory` closure references that one shared instance. Now consistent with `zit.py`'s own already-correct process-scoped pipeline-assembly cache. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 32 | `qwen3.py::load()`'s tokenizer selection | `P22-C2` | Hardcoded the production, ~151k-token tokenizer path unconditionally, regardless of which checkpoint was loaded. Against the tiny fixture (`vocab_size=128`), any token ID above 128 indexed outside `embed_tokens`'s table (`IndexError`). This exact mismatch was already hit once during `P24-A2` and "fixed" only inside `test_nodes_encoder.py`'s test harness (a post-`load()` tokenizer swap) — `load()` itself was never touched, so nothing outside that specific test harness ever worked. | Manual retrofit patch (not a numbered task) — `qwen3.py` gained `_load_tokenizer_matching_vocab(vocab_size)`, trying each vendored tokenizer in turn and returning whichever one's `len(tokenizer)` matches the checkpoint's inferred `vocab_size`, raising a clear `RuntimeError` naming every candidate tried if none match. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 33 | `worker/assets/qwen3_tiny_tokenizer/`'s file format | `P24-A2` | `tokenizer_config.json` declares `"tokenizer_class": "BertTokenizer"`, which requires a plain-text `vocab.txt` (one token per line). `P24-A2` committed `vocab.json` (a JSON dict — the BPE-tokenizer format, not `BertTokenizer`'s) and an empty, irrelevant `merges.txt`. With no `vocab.txt` present, `BertTokenizer.from_pretrained()` silently fell back to only the 5 declared special tokens (`len(tokenizer) == 5`, not 128) — undetected because `P24-A2`'s own real-mode test's prompt words all degenerated to `<unk>` (a valid index regardless of vocab size), so the test passed while testing nothing about real tokenization. | Manual retrofit patch (not a numbered task), surfaced while fixing item 32 above — the same 128-token vocabulary converted from `vocab.json` into a correctly-formatted `vocab.txt`; `vocab.json`/`merges.txt` removed. Verified directly: `len(tokenizer) == 128`. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 34 | `EmptyLatent`'s real-branch latent dtype | `P24-C2` | `torch.zeros(latent_shape, device=ctx.device)` with no `dtype=` defaults to `float32`. The loaded `ZiTModel` is materialized at `bfloat16` on CPU per `ANVILML_DESIGN.md §11.5`'s fixed precedence (fp8 always fails CPU's capability probe). `input_proj`, the first layer touched, rejected the fp32 input (`RuntimeError: mat1 and mat2 must have the same dtype`). No task before `P24-F1` ran `EmptyLatent`'s output through a real model's `forward()`. | Manual retrofit patch (not a numbered task) — reads `next(model.parameters()).dtype` from the already-loaded model (already a required real-mode input) and allocates the latent tensor at that dtype. Confirmed sufficient by tracing the rest of `zit.py::forward()`, which already does `.to(x.dtype)` on the timestep embedding before use. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 35 | `zit.py::sample()`'s missing `scheduler.scale_model_input()` call | `P21-B2` | The denoising loop called `pipeline.model(latent, t/1000.0, ...)` with the raw, unscaled latent every step. `EulerDiscreteScheduler` requires scaling the sample by the current timestep's sigma before the model call; omitting it doesn't raise an exception, it silently produces incorrect denoising — diffusers only surfaces a `UserWarning` per step, easy to miss in a busy log stream. Every prior run that reached the sampling loop produced *an* image, just a silently wrong one. | Manual retrofit patch (not a numbered task) — `scaled_latent = scheduler.scale_model_input(latent, t)` computed once per timestep and passed to both the unconditional and conditional model calls; the unscaled `latent` is left untouched for `scheduler.step()`, which correctly expects the previous unscaled sample. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
| 36 | `SaveImage`'s real branch assumed a bare `Image` | `P24-D2` | `VaeDecode`'s real branch (`P24-B2`) is deliberately batch-capable and always returns `list[PIL.Image.Image]`, even for `batch_size=1` — tested and intentional (`test_vae_decode_real_batched_latent` asserts 2 images for a batch-2 latent). `SaveImage`'s real branch calls `.save()` directly on `inputs["image"]`, assuming a bare `Image` — also tested and intentional in isolation. Neither node was wrong alone; no task before `P24-F1` wired a real `VaeDecode → SaveImage` edge end to end. | Manual retrofit patch (not a numbered task) — `SaveImage` now accepts either a bare `Image` (unchanged) or a `list[Image]` (new — takes the first, warns if more than one is dropped). An initial attempt fixed this the other way, unwrapping inside `VaeDecode`; reverted before finalizing since it broke `VaeDecode`'s own deliberately-tested batch-list contract. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md`. |
...
All twenty-seven are closed in the current task graph and source tree as of this document's generation. Items 1–10 are retrofit tasks in Phase 900 (completed phases P1/P3/P6 cannot be edited directly per project convention — fixes for defects in completed phases always land in P900). Items 11–15 are in-phase corrections to Phase 8, Phase 18, and Phase 17 respectively, since none of those phases had executed yet when found — the fix lands in the phase that owns the gap rather than in a retrofit. Items 16–22 and 27 are direct source-code retrofit patches to completed code (`P8-E3` for 16-18, `P7-B2` for 19, `install_worker_deps.sh`/`.ps1` for 20, `test_capability.py` for 21, `anvilml-hardware/Cargo.toml` for 22, `backend/Cargo.toml` for 27), applied outside the numbered task sequence (the same category as commit `0fdf3e9`) rather than as new tasks. Item 19 is the sole exception to the "documentation-vs-implementation correction, no design decision to make" characterization of 16-18, 20-22, and 27 — it required a genuine synchronization redesign (the `watch`-channel signal), not just writing the code a comment already promised; it is included here rather than deferred to a dedicated Phase-7 task because `P8-E6`'s upcoming respawn loop depends on multi-generation crash simulation that would otherwise inherit this exact deadlock risk on day one. Item 23 is categorically distinct from all prior items: it is the only entry closed by a documentation correction alone, with no source-code change of any kind — the underlying behavior was never wrong, only the design record's field-count claim. Items 24 and 25 are in-phase corrections to Phase 12, since it had not fully executed (`P12-B1` outstanding) when found — same category as items 11–15. Item 26, like item 23, is a documentation-only correction with no source-code change. Item 27 was found and worked around, but not fixed, during `P14-E1`'s own implementation — closed separately, immediately after, rather than left for `P16-E1`/`P17-D1` to rediscover independently. Items 29–36, all direct source-code retrofit patches to completed Phase 17/19/21/22/24/900 code, were found one at a time while actually attempting `P24-F1`'s Runnable Proof for the first time — the project's first real end-to-end job through the complete generic-node graph — each only reachable once the previous item was fixed, since no single existing test before this exercised more than one or two adjacent real nodes in the same chain at once. Unlike items 1–22/24/25/27/28, no corresponding numbered Forge tasks were authored for items 29–36; per the project owner's explicit decision, a synthetic post-hoc task-graph entry for this kind of debugging session was judged to have no significant relevance. See `docs/ADDENDUM_P903_QWEN3_TOKENIZER_VOCAB_MISMATCH.md` for the full narrative account.
