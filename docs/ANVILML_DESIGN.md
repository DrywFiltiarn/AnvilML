...
| `fp8` | Same pattern at `torch.float8_e4m3fn`, constructed via float32 then `.to(dtype)` (constructing directly in dtype fails on `nn.Linear`'s internal `kaiming_uniform_` RNG kernel, which isn't implemented for 8-bit float types on any backend — an unrelated limitation, not a signal about hardware fp8 support either way). **`probe_capabilities()` explicitly excludes `device_type == "cpu"` from this probe** rather than trusting its try/except result — current torch builds execute a float8 forward pass on CPU without raising (confirmed directly, across two independent environments with zero shared state — native ROCm-torch's CPU fallback, and a separate pure-CPU build under WSL — both identical), but only via internal upcast-compute-downcast emulation, which torch does not expose as distinguishable from genuine hardware support at the Python level. `fp8 = False` on CPU remains the correct, expected result — no general-purpose CPU has dedicated fp8 execution units, which is a categorical fact about the architecture class, not the per-SKU device-table guessing this table's own header warns against. (This corrects an earlier version of this row, which claimed CPU "always raises `NotImplementedError`" — true for the torch version investigated at the time, not for the current one; the conclusion was right, the stated mechanism no longer is.) |
