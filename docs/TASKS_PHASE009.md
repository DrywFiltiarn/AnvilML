...
Create worker/ package: __init__.py, nodes/__init__.py, tests/__init__.py. Create worker/requirements/base.txt (msgpack>=1.0, pillow>=10.0, pytest>=8.0). Create worker/ipc.py: at module top, if sys.platform=='win32': msvcrt.setmode O_BINARY on stdin+stdout. read_frame(): read-fully 4 bytes big-endian len then N bytes, msgpack.unpackb(raw=False). write_frame(msg): msgpack.packb(use_bin_type=True), 4-byte big-endian prefix, write to sys.stdout.buffer, flush. pytest worker/tests/test_ipc.py exits 0 (BytesIO round-trip).
...
Create worker/worker_main.py: argparse --worker-id --device-index. Set OMP/MKL/OPENBLAS/VECLIB thread env before imports. If ANVILML_WORKER_MOCK=1 skip torch import. Message loop reading frames: InitializeHardware->send Ready{worker_id,device_index,vram_total_mib(8192 mock)}; Ping{seq}->Pong{seq}; MemoryQuery->MemoryReport(0,0); Shutdown->send Dying{reason:shutdown}, flush, exit 0. Background thread sending MemoryReport every 10s. Verify: ANVILML_WORKER_MOCK=1 echo-style manual run; full proof via P9-A5 REST.
