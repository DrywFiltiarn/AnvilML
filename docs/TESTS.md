...
## test_can_handle_qwen3 (worker.nodes.arch.clip.qwen3)

**File:** `worker/tests/test_arch_clip_qwen3.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring mock mode is active (not strictly required for a pure string comparison, but consistent with the test file convention).
**Tests:** Call ``can_handle("qwen3")`` and assert the result is ``True``.
**Inputs:** ``clip_type="qwen3"``.
**Expected output:** ``can_handle("qwen3") == True`` — the Qwen3 arch module claims this clip type.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_qwen3.py::test_can_handle_qwen3 -v` exits 0.

## test_can_handle_non_qwen3 (worker.nodes.arch.clip.qwen3)

**File:** `worker/tests/test_arch_clip_qwen3.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring mock mode is active.
**Tests:** Call ``can_handle("clip_l")`` and ``can_handle("t5")``, assert both return ``False``.
**Inputs:** ``clip_type="clip_l"`` and ``clip_type="t5"``.
**Expected output:** ``can_handle("clip_l") == False`` and ``can_handle("t5") == False`` — the Qwen3 arch module does not claim these clip types.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_qwen3.py::test_can_handle_non_qwen3 -v` exits 0.

## test_load_mock_returns_realclip (worker.nodes.arch.clip.qwen3)
...
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_qwen3.py::test_load_mock_returns_realclip -v` exits 0.

## test_load_mock_no_torch_import (worker.nodes.arch.clip.qwen3)

**File:** `worker/tests/test_arch_clip_qwen3.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture.
**Tests:** Remove ``torch`` from ``sys.modules`` (if present), remove the module from cache, and re-import the ``qwen3`` module. Assert that no ``ImportError`` is raised and that ``torch`` is not in ``sys.modules`` after import — proving no top-level import of torch occurs.
...
## test_can_handle_t5 (worker.nodes.arch.clip.t5)

**File:** `worker/tests/test_arch_clip_t5.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring mock mode is active (not strictly required for a pure string comparison, but consistent with the test file convention).
**Tests:** Call ``can_handle("t5")`` and assert the result is ``True``.
**Inputs:** ``clip_type="t5"``.
**Expected output:** ``can_handle("t5") == True`` — the T5 arch module claims this clip type.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_t5.py::test_can_handle_t5 -v` exits 0.

## test_can_handle_non_t5 (worker.nodes.arch.clip.t5)

**File:** `worker/tests/test_arch_clip_t5.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring mock mode is active.
**Tests:** Call ``can_handle("qwen3")`` and ``can_handle("clip_l")``, assert both return ``False``.
**Inputs:** ``clip_type="qwen3"`` and ``clip_type="clip_l"``.
**Expected output:** ``can_handle("qwen3") == False`` and ``can_handle("clip_l") == False`` — the T5 arch module does not claim these clip types.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_t5.py::test_can_handle_non_t5 -v` exits 0.

## test_load_mock_returns_realclip (worker.nodes.arch.clip.t5)
...
**Inputs:** ``model_id="/fake/path"``, ``torch_dtype=None``.
**Expected output:** ``isinstance(result, RealClip)`` is ``True``, ``isinstance(result.tokenizer, MockTokenizer)`` is ``True``, and ``isinstance(result.text_encoder, MockTextEncoder)`` is ``True``.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_arch_clip_t5.py::test_load_mock_returns_realclip -v` exits 0.

## test_load_mock_no_torch_import (worker.nodes.arch.clip.t5)

**File:** `worker/tests/test_arch_clip_t5.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture.
**Tests:** Remove ``torch`` from ``sys.modules`` (if present), remove the module from cache, and re-import the ``t5`` module. Assert that no ``ImportError`` is raised and that ``torch`` is not in ``sys.modules`` after import — proving no top-level import of torch occurs.
**Inputs:** torch removed from ``sys.modules`` before import.
**Expected output:** Module imports successfully and ``"torch"`` is absent from ``sys.modules``, confirming mock-mode import isolation.
...
**Tests:** Import and reload the ``sampler`` module, then assert all six metadata attributes on ``EmptyLatent``: ``NODE_TYPE == "EmptyLatent"``, ``CATEGORY == "Latents"``, ``DISPLAY_NAME == "Empty Latent"``, ``DESCRIPTION`` is a non-empty string, ``INPUT_SLOTS`` has four specs (width, height, batch_size, model), and ``OUTPUT_SLOTS`` has one spec. Verify each slot's name, type, and optional flag.
**Inputs:** None (class-level inspection after reload).
**Expected output:** ``INPUT_SLOTS`` has 4 specs: width (INT, required), height (INT, required), batch_size (INT, optional), model (MODEL, optional). ``OUTPUT_SLOTS`` has 1 spec: latent (LATENT, required).
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_nodes_sampler.py::test_emptylatent_metadata_attributes -v` exits 0.
...
**File:** `worker/tests/test_nodes_sampler.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring the mock code path is taken. NODE_REGISTRY is cleared by the ``registry_clean`` fixture.
**Tests:** Import and reload the ``sampler`` module, instantiate ``EmptyLatent`` with a ``mock_context``, call ``execute(width=512, height=512, batch_size=4)``, and assert the returned dict contains a ``MockLatent`` with the correct dimensions.
**Inputs:** ``width=512``, ``height=512``, ``batch_size=4``.
**Expected output:** ``result["latent"]`` is a ``MockLatent`` with ``width==512``, ``height==512``, ``batch_size==4``.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_nodes_sampler.py::test_emptylatent_execute_returns_mock_latent -v` exits 0.
...
**File:** `worker/tests/test_nodes_sampler.py`
**Context:** ``ANVILML_WORKER_MOCK=1`` is set by the ``conftest.py`` autouse fixture, ensuring the mock code path is taken. NODE_REGISTRY is cleared by the ``registry_clean`` fixture.
**Tests:** Call ``execute()`` without providing a ``batch_size`` key in the inputs dict. The mock code path should default to 1.
**Inputs:** ``width=512``, ``height=512`` (no ``batch_size``).
**Expected output:** ``result["latent"].batch_size == 1`` — the default batch size.
**Acceptance command:** `ANVILML_WORKER_MOCK=1 worker/.venv/bin/python -m pytest worker/tests/test_nodes_sampler.py::test_emptylatent_default_batch_size -v` exits 0.
...
**File:** `worker/tests/test_nodes_sampler.py`
**Context:** NODE_REGISTRY is cleared by the ``registry_clean`` fixture. The ``@register`` decorator runs on import.
**Tests:** Re-import the ``sampler`` module so ``@register`` runs against the now-empty NODE_REGISTRY. Assert that ``"EmptyLatent"`` is a key in ``NODE_REGISTRY`` and that the registered class has the correct ``NODE_TYPE``.
...
**File:** `worker/tests/test_arch_zit.py`
**Context:** ``ANVILML_WORKER_MOCK`` is temporarily set to ``"0"`` by this test, overriding the autouse fixture. A mock ``pipeline_cache`` is provided with a ``get_or_load`` mock. The test imports ``MagicMock`` from ``unittest.mock`` inside the test body to avoid importing it at module level. The mock pipeline's ``__call__`` is configured to return ``[MagicMock(), seed]`` matching ``return_dict=False`` output format. A separate mock ``clip`` object with ``.tokenizer`` and ``.text_encoder`` attributes is passed to ``sample()``; ``loader_fn`` reads these from ``clip`` (not ``conditioning``).
**Tests:** Call ``sample()`` in real mode with a mock model that carries ``model_id="test_model"``, a mock conditioning object with only ``positive``/``negative`` attributes, a mock ``clip`` object with ``tokenizer``/``text_encoder``, and a mock ``pipeline_cache``. Assert that ``get_or_load`` was called with a key containing ``":pipeline"`` and that the returned tuple has the correct structure ``(latent_result, seed)``.
**Inputs:** ``model_id="test_model"``, ``steps=4``, ``seed=42``, ``cfg=7.0``, ``device="cpu"``, ``clip=mock_clip``.
...
**File:** `worker/tests/test_arch_zit.py`
**Context:** ``ANVILML_WORKER_MOCK`` is temporarily set to ``"0"`` by this test, overriding the autouse fixture. A mock ``pipeline_cache`` returns a ``MagicMock`` pipeline object whose ``__call__`` is configured to return ``[MagicMock(), seed]``. A separate mock ``clip`` object with ``.tokenizer`` and ``.text_encoder`` attributes is passed to ``sample()``; ``loader_fn`` reads these from ``clip`` (not ``conditioning``). Verifies the pipeline is called with all expected keyword arguments.
**Tests:** Call ``sample()`` in real mode with a mock model (``arch="zit"``, ``model_id="test_model"``), mock conditioning (with only ``positive``/``negative`` attributes), a mock ``clip`` object with ``tokenizer``/``text_encoder``, mock VAE, and a ``threading.Event()`` as ``cancel_flag``. Assert that the mock pipeline's ``__call__`` was invoked with ``output_type="latent"``, ``return_dict=False``, ``num_inference_steps=steps``, ``guidance_scale=cfg``, and a callable ``callback_on_step_end``. Also assert the returned tuple has the correct structure.
**Inputs:** ``model_id="test_model"``, ``steps=8``, ``cfg=7.5``, ``seed=99``, ``device="cuda"``, ``clip=mock_clip``.
