...
**Tests:** Call ``sample()`` in real mode with a mock model (``arch="zit"``, ``model_id="test_model"``), mock conditioning (with only ``positive``/``negative`` attributes), a mock ``clip`` object with ``tokenizer``/``text_encoder``, and a ``threading.Event()`` as ``cancel_flag``. Assert that the mock pipeline's ``__call__`` was invoked with ``output_type="latent"``, ``return_dict=False``, ``num_inference_steps=steps``, ``guidance_scale=cfg``, and a callable ``callback_on_step_end``. Also assert the returned tuple has the correct structure.
