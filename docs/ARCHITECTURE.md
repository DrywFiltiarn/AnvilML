...
AnvilML is the Rust backend binary (`sindristudio`) of the SindriStudio image-generation
platform. It:
...
└── docs/
    ...
    ├── PHASES.md                 # Phase registry (M0–M6 milestones)
    ├── TASKS_PHASE001.md         # Phase 001 narrative + task descriptions
    ├── TASKS_PHASE002.md         # Phase 002 narrative + task descriptions
    └── TASKS_PHASE003.md         # Phase 003 narrative + task descriptions
```
...
| `GET` | `/v1/system/hardware` | Hardware info (`HardwareInfo`) |
| `GET` | `/v1/system/env` | Python env preflight report |
| `GET/POST` | `/v1/jobs` | List jobs / submit new job |
| `GET/DELETE` | `/v1/jobs/:id` | Get job / cancel job |
...
| `GET` | `/v1/artifacts/:hash` | Fetch artifact image bytes |
| `GET (WS)` | `/v1/ws` | WebSocket event stream |

Full request/response shapes: `ANVILML_DESIGN.md §10`.

**WebSocket events** (`WsEvent`): `job.queued`, `job.started`, `job.progress`,
`job.image_ready`, `job.completed`, `job.failed`, `job.cancelled`, `worker.state_changed`,
...
parallel with the Linux jobs. Full spec: `TASKS_PHASE001.md §P1-A2, P1-A3`.
