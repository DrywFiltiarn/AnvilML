...
//! listing.
//!
//! `ArtifactStore` lives in its own crate because it is shared by
//! `anvilml-scheduler` (which persists `WorkerEvent::ImageReady` payloads)
//! and `anvilml-server` (which serves artifacts over HTTP), and neither of
//! those crates may depend on the other. This mirrors `anvilml-registry`,
//! which exists as its own crate for the same reason (shared by
