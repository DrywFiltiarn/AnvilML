...
/// Python runtime environment report collected at worker startup.
///
/// The scheduler uses this to verify that the worker's Python and PyTorch
/// versions are compatible with the server's expectations.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ToSchema)]
pub struct EnvReport {
    /// The Python interpreter version string (e.g. `"3.12.3"`).
    pub python_version: String,
    /// The PyTorch version string if `import torch` succeeded, or `None` if
    /// the import failed.
    pub torch_version: Option<String>,
    /// Whether `import torch` succeeded at runtime.
