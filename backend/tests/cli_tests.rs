...
/// The actual port is detected via `lsof` (the OS-assigned port cannot be
/// known in advance).
...
///   - `lsof` is available on the system.
///
/// Acceptance command:
...
/// bound port via `lsof`, hit the health endpoint, and assert HTTP 200 with
/// `"status":"ok"`.
///
